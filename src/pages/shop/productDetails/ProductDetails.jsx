...
import { useCart } from "../../../context/Contexcart"; // Import the cart context
...
useEffect(() => {
  ...
  useEffect(() => {
    if (productDetail) {
      ...
      const productImages =
        detail.productgallers?.length > 0
      ...
      const transformedProduct = {
        ...
        price: parseFloat(
          detail.is_on_sale ? detail.on_sale_price : detail.regular_price || 0
        ),
        originalPrice: detail.is_on_sale
          ? parseFloat(detail.regular_price)
          : null,
        ...
        details: {
          ...
          moreInformation: (() => {
            ...
            if (variant) {
              ...
              if (variant.customAttributes) {
                ...
                Object.entries(variant.customAttributes).forEach(
                  ([key, value]) => {
          ...
          productRating: `${averageRating.toFixed(1)}/5 stars${
            reviews.length ? ` based on ${reviews.length} reviews` : ""
          }`,
          contactSeller: detail.stores
            ? `Contact ${detail.stores.name} at ${
                detail.stores.email || detail.stores.phone
              }`
            : "Contact seller",
  ...
  const handleAddToCart = async () => {
    ...
    try {
      console.log("1. Starting add to cart...");
      ...
      console.log("2. Add to cart response:", response);

      // Show success message
      alert(t("shop.productDetails.addedToCart"));
      
      console.log("3. Refetching product data...");
      
      // Refetch product data to update stock
      try {
        await refechingProduct();
        console.log("4. Product data successfully refetched");
      } catch (refetchError) {
        console.error("Error refetching product:", refetchError);
      }

      console.log("5. Refetching cart data...");
      
      // Refetch cart data using context
      try {
        await refetchCart();
        console.log("6. Cart data successfully refetched");
      } catch (cartRefetchError) {
        console.error("Error refetching cart:", cartRefetchError);
      }
  ...
  const tabs = [
    ...
    {
      id: "shortDescription",
      label: t("shop.productDetails.tabs.shortDescription"),
    },
    {
      id: "moreInformation",
      label: t("shop.productDetails.tabs.moreInformation"),
    },
  ...
  const renderTabContent = () => {
    ...
    if (
      activeTab === "contactSeller" ||
      activeTab === "otherSellers" ||
      activeTab === "storePolicy"
    ) {
      return (
        ...
        <div className="product-rating-section">
          {product.rating ? (
            <div className="rating-display">
              ...
              <div className="rating-text">{product.details.productRating}</div>
            </div>
          ) : (
    ...
    return (
      <p>
        {product.details[activeTab] || t("shop.productDetails.noInformation")}
      </p>
    );
  };
          ...
          <div className="flex flex-col md:flex-row gap-8">
            ...
            <div className="md:w-1/2">
              ...
              <div className="flex flex-wrap gap-4 text-sm text-gray-500 mb-4">
                ...
                {product.condition && (
                  <span>
                    {t("shop.productDetails.productInfo.condition")}:{" "}
                    {product.condition}
                  </span>
                )}
              </div>
              ...
              {product.rating > 0 && (
                <div className="flex items-center gap-2 mb-6">
                  ...
                  <button
                    ...
                    onClick={() =>
                      document.getElementById("review-form-modal")?.showModal()
                    }
                  >
              ...
              {product.description && (
                <div className="mb-6">
                  <h3 className="text-lg font-medium text-gray-900 mb-2">
                    {t("shop.productDetails.productInfo.overview")}
                  </h3>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-3 flex flex-wrap gap-3">
                        ...
                        {product.store.contact?.phone && (
                          <a
                            href={`tel:${product.store.contact.phone.replace(
                              /\D/g,
                              ""
                            )}`}
                            className="inline-flex items-center text-sm text-blue-600 hover:text-blue-800 hover:underline transition-colors"
                      ...
                      <div className="mt-2 flex items-center">
                        ...
                        {related.originalPrice > related.price && (
                          <span className="ml-2 text-xs font-medium text-red-600">
                            {Math.round(
                              (1 - related.price / related.originalPrice) * 100
                            )}
                            % OFF
                          </span>
                        )}
