...
import { useVerifyStripeOrderMutation } from "../../stores/apiSlice";
...
const OrderSuccess = () => {
  ...
  useEffect(() => {
    const verifyOrder = async () => {
      ...
      try {
        console.log("1. Starting order verification...");
        ...
        console.log("2. Order verification response:", response);

        if (response.success) {
          setOrderDetails(response.order);
          
          // Clear cart after successful order verification
          console.log("3. Refetching cart data...");
          try {
            const cartResult = await refetchCart();
            if (cartResult.success) {
              console.log("4. Cart data successfully refetched");
            } else {
              console.error("Failed to refetch cart:", cartResult.error);
            }
        ...
        setError(err?.data?.message || "Failed to verify order. Please contact support.");
      } finally {
        setLoading(false);
      }
    };
    ...
    verifyOrder();
  }, [sessionId, orderId, verifyStripeOrder, refetchCart]);
